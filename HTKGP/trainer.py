    ...
    def train(self, early_stop=False):
        ...
        for epoch in range(1, self.params.ne + 1):
            ...
            while not last_batch:
                ...
                heads, rels, tails,  tim, years, months, days = self.dataset.nextBatch(self.params.bsize, neg_ratio=self.params.neg_ratio)
                last_batch = self.dataset.wasLastBatch()
                
                scores = self.model(rels,heads,  tails,years, months, days)
                ...
                loss0 = loss_f(scores_reshaped, l)
                loss1 = torch.norm(self.model.module.ent_embs_h.weight, p=2) + torch.norm(self.model.module.ent_embs_t.weight, p=2) + torch.norm(self.model.module.rel_embs_f.weight,p=2) + torch.norm(self.model.module.rel_embs_i.weight, p=2)+torch.norm(self.model.module.tim_embs.weight, p=2)
                loss = loss0 + loss1 * 0.001
                loss.backward()
                optimizer.step()
