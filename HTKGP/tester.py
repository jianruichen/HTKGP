...
class Tester:
    ...
    def replaceAndShred(self, fact, raw_or_fil, head_or_tail):
        head, rel, tail, tim,years, months, days = fact
        if head_or_tail == "head":
            ret_facts = [(i, rel, tail, tim,years, months, days) for i in range(self.dataset.numEnt())]
        if head_or_tail == "tail":
            ret_facts = [(head, rel, i, tim,years, months, days) for i in range(self.dataset.numEnt())]
        ...
        for i, fact in enumerate(self.dataset.data[self.valid_or_test]):
            ...
            for raw_or_fil in settings:
                for head_or_tail in ["head", "tail"]:     
                    heads, rels, tails, tim,years, months, days = self.replaceAndShred(fact, raw_or_fil, head_or_tail)
                    sim_scores = self.model(rels,heads,  tails, years, months, days).cpu().data.numpy()
